...
export async function upsertCompanyFromIngest(
	ctx: any,
	input: CompanyIngest,
): Promise<{ companyDocId: Id<"companies">; inserted: boolean }> {
	const now = Date.now();
...
export const listJobsByCompanyId = query({
	...
	handler: async (ctx, { companyId, paginationOpts }) => {
		const company = await ctx.db.query("companies").withIndex("by_companyId", (q) => q.eq("companyId", companyId)).unique();
		if (!company) {
