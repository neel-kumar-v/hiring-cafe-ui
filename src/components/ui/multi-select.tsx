...
import { CheckIcon, ChevronDown, Plus, WandSparkles, XIcon } from "lucide-react";
 ...
 */
const multiSelectVariants = cva("transition-all ease-in-out duration-300 p-1 px-2", {
  variants: {
    variant: {
...
export const MultiSelect = React.forwardRef<HTMLButtonElement, MultiSelectProps>(
  (
    ...
    const toggleOption = (option: string) => {
      const newSelectedValues = currentSelectedValues.includes(option) ? currentSelectedValues.filter((value) => value !== option) : [...currentSelectedValues, option];
      setSelectedValues(newSelectedValues);
      onValueChange(newSelectedValues);
    };
    ...
    const availableOptions = options.filter((option) => !currentSelectedValues.includes(option.value));
      ...
      <div className="relative">
        <Popover open={isPopoverOpen} onOpenChange={setIsPopoverOpen} modal={modalPopover}>
          ...
          <PopoverContent className="w-(--radix-popover-trigger-width) p-0" align="start" onEscapeKeyDown={() => setIsPopoverOpen(false)}>
            <Command>
              <CommandInput placeholder="Search..." value={searchValue} onValueChange={setSearchValue} onKeyDown={handleInputKeyDown} />
              <CommandList>
                ...
                <CommandGroup>
                    ...
                    <CommandItem key="all" onSelect={toggleAll} className="cursor-pointer">
                      <div
                        className={cn(
                          ...
                          currentSelectedValues.length === options.length ? "bg-primary text-primary-foreground" : "opacity-50 [&_svg]:invisible"
                        )}
                      >
                  ...
                  {showAddCustomOption && (
                    <CommandItem key="add-custom" onSelect={handleAddCustomValue} className="cursor-pointer">
                ...
                <CommandGroup>
                  <div className="flex items-center justify-around">
                    ...
                    <CommandItem onSelect={() => setIsPopoverOpen(false)} className="flex-1 justify-center cursor-pointer max-w-1/2">
                      Close
                    </CommandItem>
                  </div>
                </CommandGroup>
