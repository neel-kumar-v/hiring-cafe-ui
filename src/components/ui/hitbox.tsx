...
const hitboxVariants = cva("relative [--size-default:12px] [--size-lg:16px] [--size-sm:8px] after:absolute after:content-['']", {
  variants: {
    size: {
    ...
    position: {
      ...
      horizontal: "after:[bottom:0] after:[left:calc(-1*var(--size))] after:[right:calc(-1*var(--size))] after:[top:0]",
    },
    radius: {
      none: "",
      sm: "after:rounded-sm",
...
interface HitboxProps extends React.ComponentProps<typeof SlotPrimitive.Slot>, Omit<VariantProps<typeof hitboxVariants>, "size"> {
  size?: DynamicSize;
}
