...
function TooltipContent({ children, arrow = true, blur = true }: TooltipContentProps) {
  const { setContent, setArrow, setBlur } = useTooltip();
  React.useEffect(() => {
    setContent(children);
    setArrow(arrow);
    setBlur(blur);
  }, [children, setContent, setArrow, setBlur, arrow, blur]);
  return null;
}
...
function TooltipTrigger({ children }: TooltipTriggerProps) {
  const { content, side, sideOffset, align, alignOffset, id, arrow, blur } =
    useTooltip();
  ...
  }, [showTooltip, content, side, sideOffset, align, alignOffset, id, arrow, blur]);
  ...
  React.useEffect(() => {
    ...
    if (currentTooltip.content === content && currentTooltip.arrow === arrow && currentTooltip.blur === blur)
      return;
  ...
  }, [content, arrow, blur, currentTooltip?.id]);
...
export {
  ...
  useGlobalTooltip,
  useTooltip, type TooltipContentProps, type TooltipProps, type TooltipProviderProps, type TooltipTriggerProps
};
