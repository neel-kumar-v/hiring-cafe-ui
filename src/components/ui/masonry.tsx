...
function onDeepMemo<T extends unknown[], U>(constructors: CacheConstructor[], fn: (...args: T) => U): (...args: T) => U {
...
function usePositioner(
  ...
  const initPositioner = React.useCallback((): Positioner => {
    ...
    const computedColumnCount = columnCount || Math.min(Math.floor((layoutWidth + columnGap) / (columnWidth + columnGap)), maxColumnCount || Number.POSITIVE_INFINITY) || 1;
    const computedColumnWidth = Math.max(
      1,
      Math.floor((layoutWidth - columnGap * (computedColumnCount - 1)) / computedColumnCount)
    );
...
function useScroller({
  ...
  const [scrollY, setScrollY] = useThrottle(typeof globalThis.window === "undefined" ? 0 : (globalThis.window.scrollY ?? document.documentElement.scrollTop ?? 0), {
    fps,
    leading: true,
  });
...
function MasonryViewport(props: DivProps) {
  ...
  const validChildren = React.Children.toArray(children).filter(
    (child): child is React.ReactElement<MasonryItemPropsWithRef> => React.isValidElement(child) && (child.type === MasonryItem || child.type === MasonryItem)
  );
  ...
  if (layoutOutdated && mounted) {
    const batchSize = Math.min(
      ...
      Math.ceil(((context.scrollTop + overscanPixels - shortestColumnSize) / context.itemHeight) * context.positioner.columnCount)
    );
