      ...
      {ReactDOM.createPortal(
        <RootPrimitive
          ...
          className={cn(
            ...
            orientation === "horizontal" ? "flex flex-row items-center gap-2 px-2 py-1.5" : "flex flex-col items-start gap-2 px-1.5 py-2",
            className
          )}
          style={{
            [side]: `${sideOffset}px`,
...
function ActionBarSelection(props: DivProps) {
  ...
  return (
    <SelectionPrimitive
      ...
      {...selectionProps}
      className={cn("flex items-center gap-1 rounded-sm border-border px-2 py-1 font-medium text-sm tabular-nums", className)}
    />
  );
}
...
export { ActionBar, ActionBarClose, ActionBarGroup, ActionBarItem, type ActionBarProps, ActionBarSelection, ActionBarSeparator };
