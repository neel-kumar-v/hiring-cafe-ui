    ...
    <div className={compact ? "" : "mb-6"}>
      <div className={`flex flex-wrap ${compact ? "gap-1.5" : "gap-3"}`}>
          ...
          isDesktop ? (
            <div className={`flex flex-row flex-wrap items-center ${compact ? "gap-1" : "gap-2"}`}>
              {getLocations(workplaceCities).map((loc, index) => (
                <span
                  className={`flex items-center ${compact ? "gap-1" : "gap-2"} bg-secondary rounded-lg ${compact ? "px-2 py-1 text-sm" : "px-3 py-2"} text-foreground/80`}
                  key={index}
                >
        ...
        {getCompensation(compensation) ? (
          isDesktop ? (
            <span className={`flex items-center ${compact ? "gap-1" : "gap-2"} rounded-lg ${compact ? "px-2 py-1 text-sm" : "px-3 py-2"} bg-primary/20 text-foreground`}>
