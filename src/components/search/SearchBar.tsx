...
function SearchBarIcon({
	...
	return (
		<UniversalTooltip content={tooltipContent}>
			<Icon
				className={`h-4 w-4 cursor-pointer text-gray-400 transition-all hover:text-pink-500 ${delay} ${isVisible}`}
				onClick={onClick}
			/>
		</UniversalTooltip>
	);
			...
			<div className="-translate-y-1/2 absolute top-1/2 right-3 z-10 flex justify-end gap-1.5">
				...
				{inputValue && (
					<UniversalTooltip content="Delete Search">
						<X
							className={
								"h-4 w-4 cursor-pointer text-gray-400 transition-all hover:text-pink-500"
							}
